...
%			[dim7_idx], [old_RGB], [slice_idx], [tmp_root])
%  
...
%
%  tmp_root (optional) - directory where temporary folders for gzip
%   decompression are created. It will be set to the operating system
%   default if it is default or empty
%
%  Returned values:
%  
%  nii structure:
...
function nii = load_untouch_nii(filename, img_idx, dim5_idx, dim6_idx, dim7_idx, ...
			old_RGB, slice_idx, tmp_root)

   if ~exist('filename','var')
      error('Usage: nii = load_untouch_nii(filename, [img_idx], [dim5_idx], [dim6_idx], [dim7_idx], [old_RGB], [slice_idx], [tmp_root])');
   end
   ...
   if ~exist('tmp_root', 'var') | isempty(tmp_root)
      tmp_root = tempdir; 
   end
