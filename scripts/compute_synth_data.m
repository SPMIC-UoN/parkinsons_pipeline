function [img_synth, struc_posterior_maps] = compute_synth_data(img_nm, img_t1w, struc_weight_maps)
    if ~isequal(size(img_nm), size(img_t1w))
        error('Inputs must have the same size');
    end

    struc_posterior_maps = compute_posteriors(img_nm, struc_weight_maps);
