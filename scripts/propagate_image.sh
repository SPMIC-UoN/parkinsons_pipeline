...
if [[ -f ${nm_img_file} ]]; then
	...
	${MIRTK_BIN_DIR}/mirtk transform-image ${nm_img_file_corrected} ${REG_MNI_DIR}/${sub_id}_${type}-corrected.nii.gz -target ${TEMPLATE_DIR}/synth_template.nii.gz -dofin ${DOFS_DIR}/${sub_id}_synth-${type}_to_template_ffd.dof.gz
	echo "done"
